...
문자열의 구성을 위한 매크로 함수로 다음의 형태로 정의하였다.
#define STRING_JOB(A, B)  "A의 직업은 B입니다."

그리고는 STRING_JOB(버즈, 가수) 라는 매크로 문장이 다음의 문자열을 만들어낼 것을 기대했다.
"버즈의 직업은 가수입니다."

하지만 문자열 안에서는 매크로의 매개변수 치환이 발생하지 않아 선행처리기는 기대대로 문자열을 만들어내지 못한다.
따라서 다음 두 매크로 문장은
STRING_JOB(버즈, 가수)
STRING_JOB(하현우, 가수)
둘 다 "A의 직업은 B입니다"로 치환이 된다. 즉 A, B는 매크로의 매개변수 치환이 이루어지지 않는다. 이를 위해서 # 연산자가 필요하다.

  -문자열 내에서 매크로 매개변수 치환이 발생하게 만들기: # 연산자
# 연산자는 다음과 같은 형태로 사요이하는 연산자다.
#define STR(ABC)  #ABC

매개변수 ABC에 전달되는 인자를 문자열 "ABC"로 치환해라는 뜻이 담겨있다. 이렇듯 # 연산자는 치환의 결과를 문자열로 구성하는 연산자이다.
다음 두 문장은
STR(123)
STR(12, 23, 34)
선행처리기에 의해 각각 다음 문자열로 치환된다.
"123"
"12, 23, 34"

문자열은 나란히 선언하면, 하나의 문자열로 간주된다. 따라서 다음과 같이 문자열을 선언하는 것도 가능하다.
char * str="ABC" "DEF"; 이는 char * str="ABCDEF"; 문자열 선언과 동일하다.
따라서 다음과 같이 문장을 구성하는 것도 가능하다.
char * str=STR(12) STR(34);
그리고 이는 치환되어
char * str="12" "34"; = char * str="1234"; 문자열 선언과 동일해진다.
...
#연산자는 매크로의 매개변수 치환이 문자열로 치환되게 할 수 있다.

  -특별한 매크로 연산자 없이 단순히 연결하는 것은 불가능합니다.
대학의 학버는 일반적으로 다음과 같은 형태로 조합이 된다.

학번의 구성
10<-입학년도 65<-학과코드 175<-고유코드 --합치면--> 1065175

이에 우리는 학번을 조합하는 매크로 함수를 정의해보자면 다음과 같은 형태로 호출이 된다.
STNUM(10, 65, 175);
그리고 이 문장은 선행처리기에 의해서 다음과 같이 치환돼야 한다.
1065175

위 기능을 제공하는 매크로 함수를 첫 번째 제시는
#define STNUM(Y, S, P)  YSP

언뜻 보면 잘 정의해 놓은 것 같다. 그러나 원하는 결과를 결과를 얻지 못한다. 매개변수 Y, S, P가 붙어있기 때문에. Y, S, P의 조합이 아닌, 하나의 YSP로 인식이 된다. 결국 Y, S, P에 전달되는 값에 상관없이 YSP로 변환이 이뤄진다.
한 칸씩 띄어주면 되는 거 아닌가? 라고 생각할 수 있다. 
#define STNUM(Y, S, P)  Y S P

위와 같이 정의되면 변환과정에서 공백도 그대로 반영된다. 따라서 1065175라는 하나의 숫자로 구성되는 학번이 아닌 10, 65, 175라는 세 개의 숫자로 구성이 되는 학번이 만들어진다. 
최선의 방법은
...
#define STNUM(Y, S, P) YSP 매크로를 사용하면 printf("학번: %d\n",YSP);로 치환돼서 에러가 발생한다.
#define STNUM(Y, S, P) Y, S, P 매크로를 사용하면 printf("학번: %d\n", 10 65 175);로 치화돼서 또한 에러가 발생한다.

그렇다면 #define STNUM(Y, S, P) ((Y)*100000+(S)*1000*(p)) 매크로는 문제가 없을까?
printf("학번: %d\n", STNUM(10, 65, 075)); 문장은 두 자릿 수, 세 자릿 수인 경우도 있고 두 자릿수 또는 한 자릿수인 경우도 있다. 075의 앞에 붙는 0은 8진수의 표현을 의미하기 때문에 원하는 결과가 안 나온다.

  -필요한 형태대로 단순하게 결합하기: 매크로 ## 연산자
예제 UnivStdNum.c는 다음과 같은 기대감으로 구성된 문장이다. 
"매크로 함수에 10, 65, 075를 전달했으니까 1065075가 만들어질거다."

이러한 기대감을 만족시켜주는 연산자가 있으니, 매크로 연산자 ## 바로 그것이다. 이 연ㅅ나자는 매크로 함수의 전달인자를 다른 대상(전달인자, 숫자, 문자, 문자열등)과 이어줄 때 사용한다. 
다음 문장을 보자.

#define CON(UPP, LOW)  UPP ## 00 ## LOW

위의 매크로 몸체에는 UPP와 00 그리고 LOW가 순서대로 이어질 수 있도록 ##연산자가 사용됐다.
따라서 다음 문장을 삽입하면
int num = CON(22, 77);
이는 선행처리기에 의해서 컴파일 이전에 다음과 같이 변환한다.
int num = 220077;
이제 ## 연산자에 대해서도 알았으니, 예제UnivStdNum.c의 매크로를 다음의 매크로 함수로 바꿔서 실행해보면 정상적인 출력결과를 기대할 수 있을 것이다.
#define STNUM(Y, S, P)  Y ## S ## P
이로써 Y, S, P에 무엇이 전달되든지 단순히 연결하여 하나의 값을 얻을 수 있게 되었다. 이것이 최선의 해결책이다.
