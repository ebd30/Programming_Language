...
예제DynamicMemoryAllocation.c를 보면서 
"프로그램 종료 직전이라도 free 함수를 호출하는 걸 보니, 프로그램이 종료되어도 malloc으로 할당된 메모리 공간은 남아있게 되는 건가요?"
라는 질문을 할 수 있지만 그렇지 않다. 만약 그렇게 된다면 free함수를 호출하지 않는 예제를 계속해서 실행하면, 메모리 공간의 부족으로 운영체제의 실행에 문제가 발샐할 것이다. 하지만 그러한 일은 발생하지 않는다. 
프로그램 실행 시 할당된 모든 메모리 공간은 프로그램이 종료되면 운영체제에 의해서 전부 해제가 되기 때문이다.

"그럼 앞서 보인 예제에서는 free함수를 호출할 필요는 없겠네요?"
예제의 상황만 놓고 보면 free 함수의 호출이 불필요하다고 할 수 있다. 그러나 앞으로 구현하게 될 프로그램이 간단히 실행되고 종료되는 프로그램이 아니면 반드시 free 함수를 호출해야 한다.
습관적으로라도 malloc 함수의 호출 횟수만큼 free 함수를 호출하는 것이 좋다. 프로그램을 종료하기 직전 free 함수를 호출해서 메모리 공간을 정리한다고 해서 손해 볼 것도 없다.

  -문자열을 반환하는 함수를 정의하는 문제의 해결
당시 문제의 해결을 위해서 필요로 했던 것은 다음과 같다.
"함수가 호출 될 때마다 문자열 저장을 위한 메모리 공간의 할당이 가능해야 하고, 이 메모리 공간은 함수르 빠져나가도 소멸되지 않고 존재해야 한다."
...
malloc 함수와 free 함수를 이용하면 메모리 공간의 할당과 소멸의 시점을 프로그래머가 직접 결정할 수 있다. 때문에 전역변수나 지역변수가 감당하지 못하는 일들을 할 수 있다.
...
malloc 함수와 차이점은 메모리 공간의 할당을 위한 인자의 전달방식에 있다.
calloc 함수는 두 개의 숫자를 인자로 전달받는다. 첫 번째 전달인자로는 할당할 블록의 갯수 정보가 전달되고, 두 번째 전달인자로는 블록 하나당 바이트 크기의 정보가 전달된다.

calloc 함수의 호출 방식
calloc(4, 30) // 4byte 크기의 블록(elt_size) 30개를(elt_count) 힙 영역에 할당해 주세요.
120byte를 할당해 달라는 것과, 4byte 크기의 블록 30개를 할당해 달라는 것은 결과적으로 완전히 동일하다. 다시 말해서 calloc함수는 malloc함수와 인자를 전달하는 방식에서 차이를 보인다.
이것 말고도 한가지 차이점이 더 있는데 malloc 함수는 할당된 메모리 공간을 별도의 값으로 초기화하지 않는다. 따라서 할당된 메모리 공간이 쓰레기 값으로 채워지지만 calloc 함수는 할당된 메모리 공간의 모든 비트를 0으로 초기화시킨다.
바로 이러한 특성 때문에 calloc 함수가 대신 사용되는 경우도 많다. 
calloc 함수의 호출로 할당된 메모리 공간을 해제할 때에도 free 함수를 사용하면 된다.

  -힙에 할당된 메모리 공간 확장 시 호출하는 realloc 함수
한 번 할당된 메모리 공간은 그 크기를 확장할 수 없다. 이는 모든 영역의 메모리 공간에 해당하는 말이다. 이미 할당되어버린 배열의 길이를 늘릴 수 있냐 물으면 어느 영역에 선언을 하건 간에 이러한 일은 불가능하다.
하지만 그 영역이 힙이라면, 그리고 realloc 함수를 이용하면 가능하다.
#include <stdlib.h>
void * realloc(void * ptr, size_t size);
-> 성공 시 새로 할당된 메모리의 주소 값, 실패 시 NULL 값 반환

realloc 함수의 첫 번째 전달인자로, 확장하고자 하는 힙 메모리의 시작 주소 값을 전달한다. 
두 번째 전달인자로는 확장하고자 하는 메모리의 전체 크기를 전달한다. 즉 매개변수 ptr과 size를 이용해서 다음과 같은 요구를 할때 호출하는 것이 realloc 함수다.
...
위 코드의 실행결과는 반환 값 기준으로 두 가지로 구분된다.
